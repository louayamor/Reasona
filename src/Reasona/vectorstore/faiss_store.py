...
class FaissStore:
    ...
    Thread-safe SQLite connections (per-thread) for Flask / multi-threaded inference.
    """
    ...
    def __init__(
        ...
        self._conn_thread: Dict[int, sqlite3.Connection] = {}  # thread_id -> connection
    ...
    def _get_conn(self) -> sqlite3.Connection:
        ...
        if thread_id not in self._conn_thread:
            self._conn_thread[thread_id] = sqlite3.connect(self.db_path)
        return self._conn_thread[thread_id]
    ...
    def close(self):
        for conn in self._conn_thread.values():
            conn.close()
    ...
    def save(self):
        if self.readonly or self.index is None:
            return
    ...
    def _create_index(self, dim: int, nlist: Optional[int] = None):
        if nlist is None:
            nlist = min(4096, max(1, int(np.sqrt(self.max_vectors)))) if self.max_vectors else 1024
    ...
    def _retrain_from_db(self):
        ...
        vectors = np.vstack([np.frombuffer(r[0], dtype="float32") for r in rows])
    ...
    def search(self, query: np.ndarray, k: int) -> Tuple[List[float], List[Dict]]:
        ...
        for idx in ids[0]:
            if idx < 0:
                continue
            cur.execute(
                "SELECT chunk_id, original_id, text, source FROM metadata LIMIT 1 OFFSET ?",
                (int(idx),),
            )
            row = cur.fetchone()
            if row:
                results.append({"id": row[0], "original_id": row[1], "text": row[2], "source": row[3]})
    ...
    def is_ready(self) -> bool:
        return self.index is not None and self.count_vectors() > 0 and (not hasattr(self.index, "is_trained") or self.index.is_trained)
